...
class TimerActionHandler(
    ...
    fun toggleTimer(item: TimerItem) {
        AppExecutors.get().diskIO {
            if (item.state == TimerItem.STATE_RUNNING) {
                ...
                if (item.state == TimerItem.STATE_FINISHED || item.state == TimerItem.STATE_RINGING) stopTimerService(item.id)
                if (item.state == TimerItem.STATE_FINISHED || item.state == TimerItem.STATE_RINGING || item.remainingMillis <= 0)
    ...
    fun stopTimer(item: TimerItem) {
        AppExecutors.get().diskIO {
            stopTimerService(item.id)
