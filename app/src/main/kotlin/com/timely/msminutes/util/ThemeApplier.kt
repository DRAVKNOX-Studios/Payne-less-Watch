...
object ThemeApplier {
    ...
    private val ID_BG_COLOR = View.generateViewId()
    private val ID_TEXT_COLOR = View.generateViewId()
    ...
    private fun setSharedBackground(view: View, fillColor: Int, cornerDp: Int, strokeColor: Int = 0, strokeWidthDp: Int = 0) {
        ...
        if (view.getTag(ID_BG_COLOR) == key && view.background != null) return
    ...
    fun applyCard(view: View?, t: ThemeTokens, customTheme: Boolean) {
        if (view == null) return
        val fillColor = t.surface
        val strokeColor = (t.textPrimary and 0x00FFFFFF) or 0x33000000
        val strokeWidth = 1
        
        setSharedBackground(view, fillColor, SharedDrawablePool.CORNER_CARD, strokeColor, strokeWidth)
    }

    fun applyCardDelete(view: View?, t: ThemeTokens) {
        if (view == null) return
    ...
    fun applyPillButton(view: View?, t: ThemeTokens, accentFill: Boolean = true) {
        ...
        if (view is TextView) {
            ...
            if (view.getTag(ID_TEXT_COLOR) != textColor) {
                view.setTextColor(flatCsl(textColor))
                view.setTag(ID_TEXT_COLOR, textColor)
            }
        }
    }
    ...
    fun applyFabDrawable(fab: View?, t: ThemeTokens) {
        ...
        if (fab is TextView) {
            val textColor = t.textPrimary
            if (fab.getTag(ID_TEXT_COLOR) != textColor) {
                fab.setTextColor(flatCsl(textColor))
                fab.setTag(ID_TEXT_COLOR, textColor)
            }
        }
    }
    ...
    fun applyTextPrimary(tv: TextView?, t: ThemeTokens) {
        if (tv == null) return
        if (tv.getTag(ID_TEXT_COLOR) != t.textPrimary) {
            tv.setTextColor(t.textPrimary)
            tv.setTag(ID_TEXT_COLOR, t.textPrimary)
        }
    }

    fun applyTextSecondary(tv: TextView?, t: ThemeTokens) {
        if (tv == null) return
        if (tv.getTag(ID_TEXT_COLOR) != t.textSecondary) {
            tv.setTextColor(t.textSecondary)
            tv.setTag(ID_TEXT_COLOR, t.textSecondary)
        }
    }
    ...
    fun tintCompoundButton(cb: android.widget.CompoundButton?, t: ThemeTokens) {
