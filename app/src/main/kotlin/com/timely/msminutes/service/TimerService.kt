    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (ACTION_STOP == action) {
            ...
            if (item != null) {
                ...
                repo.update(item)
                com.timely.msminutes.widget.WidgetNotifier.notifyUpdate(this)
            }
        ...
        if (ACTION_FINISHED == action) {
            ...
            if (target == null) {
                // If we were started via startForegroundService, we MUST call startForeground
                // even if we are about to stop.
                stopSelf()
                return START_NOT_STICKY
            }
            target.state = TimerItem.STATE_RINGING
            target.remainingMillis = 0
            repo.update(target)
            com.timely.msminutes.widget.WidgetNotifier.notifyUpdate(this)

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                startForeground(NOTIF_ID, buildFinishedNotification(target), android.content.pm.ServiceInfo.FOREGROUND_SERVICE_TYPE_SPECIAL_USE)
            } else {
